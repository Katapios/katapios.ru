...
/**
 *
 * Описание файла: Контроллер входа и выхода 
 *
 * @изменён 1.9.2009
 */
...
class Lunit extends CI_Controller {
	...
	function login () {
		...
		if ($this->form_validation->run ()) {
			...
			$this->lib_view->login_page('login',array (),'Вход администратора');
		}
...
//		$data['clicks'] = $res['clicks'];
		...
		//Число основных страниц
		$data['m_pages'] = $this->db->count_all_results ('stat_pages');
		
		//Число дополнительных страниц
...
//		$data['max'] = $res['url']; //ИД ссылки с максимальной посещаемостью
