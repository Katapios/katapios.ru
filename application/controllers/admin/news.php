<?php

/**
 *
 * Описание файла: Контроллер "Ссылки" 
 *
 * @изменён 3.9.2009
 */

if (!defined('BASEPATH')) exit('No direct script access allowed');

class News extends CI_Controller {
	
	var $iname = 'new';
	
	//Конструктор
	public function __construct() {		
		parent::__construct();

		//Проверка или был вход
		$this->lib_auth->check_admin();		
		
		$mdl_name = 'mdl_'.$this->iname; //Имя модели
		
		$this->load->model ($mdl_name); //Загружаем модель
		
	}
	
	function index ($link_num = 0) {		
		
		$this->lib_mng->show_index($this->iname,'Список новостей', $link_num);
		
	}
	
	/**
	 * Добавление новой новости
	 */
 	function add () {
        $this->load->helper ('tinymce');//Загрузка хелпера для HTML-редактора
        $this->lib_mng->add ($this->iname,'Добавление новой новости');
 	}
 	
 	/**
 	 * Просмотр новости
 	 */
	function show ($id) {		
		$this->lib_mng->show ($this->iname,$id,'Просмотр новости');		
	} 

	/**
	 * Редактирование новости
	 */
 	function edit ($id)
    {
        $this->load->helper ('tinymce');//Загрузка хелпера для HTML-редактора	
        $this->lib_mng->edit ($this->iname,$id,'Изменение новости');
 	}
 	
	/**
	 * Удаление новости
	 */
 	function del ($id) {
        $this->lib_mng->del ($this->iname,$id);
 	}
	 
	/**
 	 * Сортировка  
	 */
	function sort ($field) {
		$this->lib_mng->set_sort ($this->iname, $field);
	}
	
	/**
 	 * Поиск  
	 */
	function search () {
		$this->lib_mng->do_search ($this->iname);
	} 	
	 	
	
}


?>